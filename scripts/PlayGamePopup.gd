...
func _ready():
	...
	call_deferred("update_game_mode", Main.team_mode, false)
...
func _on_CheckBox_toggled(button_pressed):
	update_game_mode(button_pressed, true)

func update_game_mode(game_mode, update_size=true):
	if game_mode:
		...
		_update_btn(update_size)
	else:
		Main.solo_mode()
		$Content/SaveGameList.hide()
		$Content/Turno.hide()
		_set_play_game_btn(update_size)
...
func _set_new_game_btn(update_size=true):
	# TODO
	#$Content/Button.set_text("Nueva Partida")
	var original_w_size = $Content.get_size()
	if update_size:
		...
		#$Content/Button.rect_position.y += y_offset
	new_game = true

func _set_play_game_btn(update_size=true):
	# TODO
